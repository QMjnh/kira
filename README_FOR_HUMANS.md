...
Kira runs on your laptop. Your source media stays on the laptop or the drive connected to it. Kira creates only the temporary files needed to move selected photos to and from the iPad, then lets you remove those when the job is finished. The optional Google Photos connection transfers only photos and videos you explicitly import or upload.

## Combine local and Google Photos media

If one theme is split between a camera folder and phone media in Google Photos, open the local folder in Kira first, then click **Add Google media here**. Choose photos and videos in Google's own picker. Kira adds them to that same local folder, skips byte-for-byte duplicates, and keeps changed same-name files separately as possible edits. If no folder is open, Kira uses its Google Photos inbox.

You can also select JPEGs or videos in Kira and click **Upload media to Google Photos**, or include videos in an iPad job in their original format. Nothing syncs automatically, and disconnecting Google Photos does not delete files already downloaded to the laptop. Put `google-oauth-client.json` in the Kira application folder beside `server.py`; moving the data folder does not affect it.
...
For installation, technical details, folder layout, and security notes, see the [technical README](README.md).
